    ...
    for (let h = 1; h <= rows; h++) {
      ...
      for (let r0 = Math.max(0, row - h + 1); r0 <= Math.min(row, rows - h); r0++) {
        for (let c0 = Math.max(0, col - w + 1); c0 <= Math.min(col, cols - w); c0++) {
          if (row >= r0 && row < r0 + h && col >= c0 && col < c0 + w) yield { r0, c0, h, w };
        }
      }
    }
  }
...
const raw = readFileSync(new URL("../src/puzzles.ts", import.meta.url), "utf8");
const jsonish = raw
  .replace(/import[\s\S]*?=\s*/, "")
  .replace(/export const PUZZLES[^=]*=\s*/, "")
  .replace(/export function[\s\S]*/, "")
  .replace(/(\w+):/g, '"$1":')
  .replace(/'/g, '"');

// Simpler: read generated.json if exists, or parse from ts manually
const data = JSON.parse(
  readFileSync(new URL("./generated.json", import.meta.url), "utf8").replace(/^\uFEFF/, "")
);

let allOk = true;
for (const [diff, list] of Object.entries(data)) {
  for (const p of list) {
    const clues = p.clues.map((c) => [c.row, c.col, c.value]);
    const sum = clues.reduce((s, [, , v]) => s + v, 0);
    const area = p.rows * p.cols;
    const ok = sum === area && solvePuzzle(p.rows, p.cols, clues);
    console.log(`${p.id} ${p.name}: sum=${sum}/${area} ${ok ? "OK" : "FAIL"}`);
    if (!ok) allOk = false;
  }
}
